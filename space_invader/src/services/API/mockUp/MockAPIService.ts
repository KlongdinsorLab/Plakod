...
export default class MockAPIService extends AbstractAPIService {
     ...
     private applyPlayerBooster(boostersId: number[]): number[] {
          ...
          boostersId.forEach((boosterId) => {
               ...
               const index: number = PlayerBoosters.findIndex(pb =>   // player booster
                    pb === playerBooster
               );
               if (index === -1) {
     ...
     private createGameSession(bossId: number, boosterDropId: number, boosterDuration: number): GameSessionSchema {
     ...
     private addPlayerBoosters(boosters: {boosterId: number, duration: number}[]): Promise<void> {
     ...
     private async getBoosterLevelUp(level: number, newLevel: number): Promise<number[]> {
          ...
          const resultBoosterId = mockBoosters.map(
               b => b.boosterId
          );
     ...
     startGameSession(boostersId: number[]): Promise<Response<GameSessionDTO>> {
          return new Promise<Response<GameSessionDTO>>(resolve => {
               ...
               const gameSession = this.createGameSession(bossId, boosterDropId, boosterDuration);

               const gameSessionDTO : GameSessionDTO = {
                    gameSessionId: gameSession.id,
                    bossId: gameSession.boss_id,
                    boosterDropId: gameSession.booster_drop_id,
                    boosterDropDuration: gameSession.booster_drop_duration,
               ...
               resolveIndex.forEach(
                    index => PlayerBoosters[index].status = "EXPIRE"
               );
     ...
     finishGameSession(gameSessionId: number, score: number, lap: number, isReceiveBooster: boolean): Promise<Response<GameSessionFinishedDTO>> {
          return new Promise<Response<GameSessionFinishedDTO>>(async resolve => {
               ...
               if (lap !== 10) {
                    throw new Error(`Can't finish game. Lap must have 10 number from lap: ${lap}`);
               }
