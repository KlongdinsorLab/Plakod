...
export class RewardPopup extends Popup {
	...
	create(): void {
		...
		this.screenOverlayHitBox = this.scene.add
			.rectangle(0, 0, this.scene.scale.width, this.scene.scale.height)
			.setOrigin(0)
			.setInteractive()
			.on('pointerup', () => {})
		...
		this.rewardPopupIcon = this.scene.add
			.image(this.scene.scale.width / 2, 112, 'icon', 'icon_achievement.png')
			.setScale(2.5)
		...
		this.rewardPopupButtonToSetting = this.scene.add
			.nineslice(
				this.scene.scale.width / 2,
				1074,
				'button',
		...
		this.rewardPopupButtonClose = this.scene.add
			.nineslice(
				this.scene.scale.width / 2,
				1194,
				'button',
