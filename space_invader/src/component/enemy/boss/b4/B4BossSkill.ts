...
export class B4BossSkill extends BossSkill {
    ...
    constructor(
        ...
        this.skill = this.scene.physics.add.image(0, 0, 'b4v1', "b4_skill1.png").setOrigin(0.5, 0.5).setVisible(false)
        this.score = score
        this.isActive = true
    }
    ...
    shootSingleCrescent() {
        if(this.isActive) new B4SingleCrescent(this.scene, this.player, this.score, this.skill.x, this.skill.y)
    }

    shootDoubleCrescent() {
        if(this.isActive) new B4DoubleCrescent(this.scene, this.player, this.score, this.skill.x, this.skill.y)
    }
}