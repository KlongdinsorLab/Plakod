...
export default class AchievementBag{
    ...
    private rewardPosition = [{x:282,y:498}, {x:474, y:498}, {x:586, y:498}]
    ...
    private totalUnlockedAchievement:number = 0
    private totalAchievement:number = 0
    private totalUnlockedReward:number = 0
    private isShownPopup:boolean[] = []
    private currentUnlockedReward:number[] = []
    private currentUnlockedRewardIndex:number = 0
    private currentUnlockedPointer:number = 0
    private isShownUnlocked : boolean = false
    private isMaxCompleted : boolean = false
    ...
    private achievementGraphics: Map<number, Phaser.GameObjects.Graphics> = new Map()
    ...
    create():void {
        ...
        for(let i=0; i<this.maxLines; i++){
            for(let j=0; j<this.maxAchievementPerLine; j++){
                ...
                let position = {
                    x: this.startPosition.x + (this.achievementSize.width + this.gapSize.width) * j, 
                    y: this.startPosition.y + (this.achievementSize.height + this.gapSize.height) * i
                }
                this.achievementUI[this.index] = 
                new AchievementUI(
                    this.scene, 
                    this.sortedAchievementJson[this.index].id, 
                    {
                        x: position.x, 
                        y: position.y, 
                        isLocked: !this.unlockedAchievementJson.find((achievement)=>achievement.id === this.sortedAchievementJson[this.index].id) ? true : false,
                    }
                )    
                this.achievementUI[this.index].create();
                ((index) => {
                    this.achievementUI[index].getBody().setInteractive().on('pointerup', () => {
    ...
    createDefaultUI():void {
        this.iconUnlocked = this.scene.add.image(
            248, 
            298, 
            'icon', 
            'icon_achievement_unlocked.png'
        )
        .setOrigin(0)
        ...
        this.textNumberUnlocked = this.scene.add.text(
            344, 
            350,
    ...
    createRewardIcon(index: number): void {
        ...
        if (!isLastIcon) {
            ...
            if (isUnlockedReward) {
                ...
                isLocked = false
                characterId = (this.totalUnlockedReward * index + 1) * this.achievementPerReward;
            }
        ...
        if (!this.isShownUnlocked && isUnlockedReward) {
            ...
            text = characterId.toString()
        } else if (this.currentUnlockedReward[this.currentUnlockedRewardIndex] !== undefined) {
            this.isShownPopup[index] = false
            isLocked = true
            warning = true
        ...
        if (this.isMaxCompleted) {
            ...
            isLocked = false
            textColor = this.rewardUnlockColor
            characterId = (this.totalUnlockedReward - 1 + index) * this.achievementPerReward
            text = characterId.toString()
        }
        ...
        if (isLastIcon) {
            ...
            this.createUnlockedRewardIcon(index, position, characterId, text, textColor, isLocked)
        }
        if(warning){
            this.rewardAlert[index] = this.scene.add.image(position.x+56, position.y-56, 'icon', 'icon_alert.png').setOrigin(1,1)
        }
    }
    createLockedRewardIcon(index: number, position: { x: number, y: number }, text: string, textColor: string): void {
        this.rewardIcon[index] = this.scene.add.image(position.x, position.y - 6, 'icon', 'icon_lock.png')
            .setOrigin(0.5, 1)
        this.createRewardText(index, position, text, textColor)
        this.createRewardHitBox(index, position, text)
    }
    createUnlockedRewardIcon(index: number, position: { x: number, y: number }, characterId: number, text: string, textColor: string, isLocked:boolean): void {
        this.rewardIcon[index] = this.scene.add.image(position.x, position.y - 40, 'mcpreview', `mcpreview_mc${(characterId / 4)+1}.png`)
            .setScale(0.35)
            .setOrigin(0.5, 1)
        this.createRewardText(index, position, text, textColor)
        this.createRewardHitBox(index, position, text)
    ...
    createRewardText(index: number, position: { x: number; y: number }, text: string, textColor: string): void {
        this.rewardText[index] = this.scene.add.text(position.x, position.y, text)
            .setOrigin(0.5, 1)
            .setFontSize(56)
            .setColor('#FFFFFF')
            .setStroke(textColor, 6);
    }
    createRewardHitBox(index: number, position: { x: number; y: number }, text: string): void {
        this.rewardHitBox[index] = this.scene.add.rectangle(position.x, position.y - 46, 100, 100);
        this.rewardHitBox[index].setInteractive().on('pointerup', () => {
            this.handleClickedReward(text, index);
        });
    }
    calculateProgressBar():void {
        let reward = this.totalUnlockedAchievement - (this.totalUnlockedReward*this.achievementPerReward)
        ...
        while(reward >= this.achievementPerReward){
            ...
            if(reward >= 0 && totalReward <= TOTAL_MC){
                this.currentUnlockedReward.push((totalReward++)*this.achievementPerReward)
            }
        }
        ...
        if(
            this.totalUnlockedAchievement/this.achievementPerReward >= this.totalAchievement/this.achievementPerReward 
            && this.totalUnlockedReward === Math.floor(this.totalUnlockedAchievement/this.achievementPerReward)
        ){
            this.isMaxCompleted = true
        }
        ...
        if(this.totalUnlockedReward >= 1){
            this.currentProgress -= (this.totalUnlockedReward-1)*this.achievementPerReward
        }
    ...
    handleClickedReward(text:string, index:number):void {
        if(text === '?') return
        let achievement = parseInt(text)
        if(this.currentUnlockedReward.includes(achievement) && !this.isShownPopup[index] && this.currentUnlockedReward[this.currentUnlockedPointer] === achievement){
            this.isShownPopup[index] = true
    ...
    handleSelectAchievement(achievementUI: AchievementUI):void {
        if(this.selectedAchievement !== undefined){
            if(this.selectedAchievement.getPosition() === achievementUI.getPosition()){
                this.setDeselected()
                return
            }else{
                this.setDeselected()
    ...
    setDescription(achievementUI: AchievementUI):void {
        let text = `achievement_description_${achievementUI.getId()}`
        let title = `achievement_title_${achievementUI.getId()}`
        this.descriptionAchievementUI = new AchievementUI(this.scene, achievementUI.getId(), {x: 96, y: 986})
        ...
        if(this.unlockedAchievementJson.find((achievement)=>achievement.id === achievementUI.getId()) === undefined){
            this.descriptionLockText = I18nSingleton.getInstance()
            .createTranslatedText(
                this.scene,
                216,
                1064,
                'mybag_achievement_locked'
            )
            .setOrigin(0)
            .setAlign('left')
    ...
    sortedAchievement():void {
        this.unlockedAchievementJson.forEach((achievement)=>{
            if(!this.sortedAchievementJson.find((sortedAchievement)=>sortedAchievement.id === achievement.id)){
    ...
    updateFont():void {
        ...
        if(this.descriptionLockText && this.descriptionLockText.active){
            this.descriptionLockText?.setStyle({
                fontFamily: 'Mali',
                fontWeight: '500',
            })
            .setSize(334, 96)
    ...
    initFontStyle():void {
        ...
        if(this.textNumberUnlocked){
            this.textNumberUnlocked?.setStyle({
            fontFamily: 'Mali',
            fontStyle: 'bold',
        })
        .setFontSize(32)
