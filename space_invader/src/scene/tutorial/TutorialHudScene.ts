...
export default class TutorialHudScene extends Phaser.Scene {
	...
	preload() {
		...
		this.load.atlas(
			'warmup',
			'assets/sprites/warmup/warmup_spritesheet.png',
			'assets/sprites/warmup/warmup_spritesheet.json',
		)
		// this.load.audio('tutorialCharge', 'sound/tutorial-charge.mp3')
	}

	init({ score, gauge, menu, reloadCount, player }: Hud) {
		this.score = score
		this.gauge = gauge
	...
	create() {
		...
		this.score
			.getLayer()
			.getAll()
			.forEach((layer) => scoreLayer.add(layer))

		const scoreBackground = this.add
			.image(
				score.x - 2 * MARGIN,
		...
		this.reloadCount
			.getLayer()
			.getAll()
			.forEach((layer) => reloadLayer.add(layer))

		const reloadBackground = this.add
			.image(
				reloadCount.x,
		...
		setTimeout(() => {
			...
			self.input.once(
				...
				() => {
					scoreLayer
						.getAll()
						.forEach((layer) => this.score.getLayer().add(layer))
					reloadLayer
						.getAll()
						.forEach((layer) => this.reloadCount.getLayer().add(layer))
					this.menu.getLayer().add(menu)
					self.scene.launch('tutorial controller', { player: this.player })
