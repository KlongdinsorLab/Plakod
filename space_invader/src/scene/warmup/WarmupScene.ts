	...
	create() {
		...
		WebFont.load({
			...
			active: function () {
				...
				const descriptionY =
					exhaleSprite.y - exhaleSprite.height / 4 - 4 * MARGIN

				const i18n = I18nSingleton.getInstance()
				const description = i18n
					.createTranslatedText(self, width / 2, descriptionY, 'warmup_hold')
				...
				const exhale = i18n
					.createTranslatedText(self, width / 2, height - 6 * MARGIN, 'exhale')
					.setWordWrapWidth(width / 2)
					.setOrigin(0.5, 0.5)
