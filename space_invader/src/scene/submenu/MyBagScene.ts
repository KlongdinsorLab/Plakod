...
export default class MyBagScene extends Phaser.Scene{
    ...
    async create(){
        ...
        const achievementData = await apiService.getUnlcokedAchievement()
        const unlockedAchievement = achievementData.response
        ...
        this.disabledBoosterButtonBackground = disabledBoosterButton.buttonBackground
        ...
        this.boosterButtonHitBox = this.add.rectangle(width/2-308, 508, 300, 100).setOrigin(0)
        ...
        this.rewardButtonHitBox = this.add.rectangle(width/2+8, 508, 300, 100).setOrigin(0)
        ...
        WebFont.load({
            ...
            active: () =>{
                ...
                self.boosterButtonText?.setStyle({
                    fontFamily: 'Mali',
                    fontStyle: 'bold',
                    color: 'white'
                })
                ...
                self.rewardButtonText?.setStyle({
                    fontFamily: 'Mali',
                    fontStyle: 'bold',
                    color: 'white'
                })
                ...
                self.disabledBoosterButtonText?.setStyle({
                    fontFamily: 'Mali',
                    fontStyle: 'bold',
                    color: 'white'
                })
                ...
                self.disabledRewardButtonText?.setStyle({
                    fontFamily: 'Mali',
                    fontStyle: 'bold',
                    color: 'white'
                })
    ...
    ):{ buttonBackground: Phaser.GameObjects.Graphics, buttonText: Phaser.GameObjects.Text }{
        const button = this.add.graphics()
        button.fillStyle(backgroundColor)
        button.fillRoundedRect(
            position.x, position.y, width, height, 
            { tl: radius?.tl || 0 , tr: radius?.tr || 0, bl: radius?.bl || 0, br: radius?.br || 0 }
        )
    ...
    createArrowButton(
        ...
        height:number,
        isFlipXIcon:boolean,
    ):{ buttonBackground: Phaser.GameObjects.NineSlice, buttonIcon: Phaser.GameObjects.Image, buttonOverlay: Phaser.GameObjects.NineSlice }{
        ...
        const buttonOverlay = this.add.nineslice(
            position.x,  
            position.y, 
            'button', 
            'button_red40.png', 
        ...
        .setTint(0x000000)
        .setDepth(1)
        return { buttonBackground:button, buttonIcon:buttonIcon, buttonOverlay:buttonOverlay }
    }
    ...
    setArrowOverlay():void{
        ...
        if(this.slotType === SlotType.BOOSTER && this.boosterBag.getTotalBooster() < this.boosterBag.getMaxBoosterPerPage()){
            this.setLeftArrowOverlay(true)
            this.setRightArrowOverlay(true)
        }
        if(this.slotType === SlotType.BOOSTER && this.boosterBag.getPageIndex()-this.boosterBag.getStartIndex() < this.boosterBag.getMaxBoosterPerPage()){
            this.setLeftArrowOverlay(true)
        }
